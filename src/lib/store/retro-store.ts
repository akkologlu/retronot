...
interface RetroState {
  ...
  // Drafts (Ghost Cards)
  drafts: Record<string, { column: string; length: number }> // userId -> draft info
  setDraft: (userId: string, draft: { column: string; length: number } | null) => void
  setDrafts: (drafts: Record<string, { column: string; length: number }>) => void

  realtimeChannel: any | null // Using any to avoid complex type imports for now, or RealtimeChannel
  setRealtimeChannel: (channel: any | null) => void
}
...
export const useRetroStore = create<RetroState>((set) => ({
  ...
  updateActionItem: (item) => set((state) => ({
    actionItems: state.actionItems.map((i) => (i.id === item.id ? item : i)),
  })),
  removeActionItem: (id) => set((state) => ({ actionItems: state.actionItems.filter((i) => i.id !== id) })),
  ...
  removeVote: (id) => set((state) => ({ votes: state.votes.filter((v) => v.id !== id) })),

  setDraft: (userId, draft) => set((state) => {
